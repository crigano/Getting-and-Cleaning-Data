...
#Features to be extracted and respective column headers.
...
# Cleans and performs analysis of both the test and train
# datasets. 
#Writes a clean.dataset to a file "cleaned.txt".
run.analysis <- function(dir) {
  p("Getting and Cleaning Data Project")
   
  # Read the data.
  p("Reading in datasets.")
  test <- get.data(dir, "test")
  train <- get.data(dir, "train")

  # Join the data data sets.
  p("Joining datasets.")
  ...
  p("Saving clean data to file:", clean.file.name)
  write.table(all.data.clean, clean.file.name, row.names = FALSE, quote = FALSE)
}

# Run the analysis.
run.analysis( paste(getwd(),"data", sep="/"))
